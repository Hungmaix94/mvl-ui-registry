...
import type { SalesRevenueImportResult } from '@/types/sales-revenue-import.ts'
import type { ImportJob } from '@/services/export-service.ts'
import type { DialogConfig } from '@/types'
import SalesRevenueImportResultContent from '../components/SalesRevenueImportResultContent.tsx'
...
export function useSalesRevenueImportResultDialog() {
  const { displayCustom, displayClose, updateConfig } = useDialog()
  const { invalidateByPrefix } = useInvalidateQueries()
  const [isOpen, setIsOpen] = useState(false)
  const [result, setResult] = useState<SalesRevenueImportResult | null>(null)
  ...
  const loadResultFiles = useCallback(async (rawFiles: ResultFiles | string | null | undefined) => {
    ...
    const parsedResult: SalesRevenueImportResult = {
  ...
  const handleClose = useCallback(() => {
    ...
    invalidateByPrefix('payroll/sales-revenues')
  }, [invalidateByPrefix])
  ...
  const createConfig = useCallback((): DialogConfig => {
    return {
      ...
      content: result ? (
        <SalesRevenueImportResultContent
          result={result}
          activeTab={activeTab}
