...
import {
  useSalesRevenueImportTemplate,
  useStartSalesRevenueImport,
  ...
  return (
    <Flex direction="column" gap="5" className="w-full">
      ...
      <FileUpload
        ...
        purpose="hrm_sales_revenue_import"
      />
    </Flex>
  )
}

export function useSalesRevenueImportUploadDialog() {
  ...
  const startImportMutation = useStartSalesRevenueImport()
  const {
    refetch: refetchTemplate,
    isFetching: isTemplateFetching,
    data: templateResponse,
  } = useSalesRevenueImportTemplate({ enabled: false })
  ...
  const handleConfirm = useCallback(async () => {
    ...
    if (!month) {
      const message = 'Vui lòng chọn doanh thu tháng'
      setUploadError(message)
      setError(message)
      throw { isValidationError: true }
    }
    ...
    try {
      ...
      // Format target_month to MM/YYYY
      ...
      const importResponse = await startImportMutation.mutateAsync({
        ...
        options: {
          ...
          allow_update: false,
          // NOTE: only this one matter, the other are default options for all imports
          handler_options: {
      ...
      // Save the successful month for next time
  ...
  const createConfig = useCallback(() => {
    return {
      title: 'Nhập danh sách Doanh thu kinh doanh',
      content: (
        <UploadContent
      ...
      leftFooterContent: ability.can('import_template', 'sales_revenue') ? (
  ...
  const openUploadDialog = useCallback(
    (initialMonth?: Date) => {
      return new Promise<string>((resolve, reject) => {
        ...
        setFileToken(null)
        setUploadError(null)
        // Use initialMonth if provided, otherwise use last successful month
