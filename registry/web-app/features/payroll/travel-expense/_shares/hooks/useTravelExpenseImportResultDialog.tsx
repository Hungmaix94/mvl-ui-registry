...
import type { TravelExpenseImportResult } from '@/types/travel-expense-import.ts'
import type { ImportJob } from '@/services/export-service.ts'
import type { DialogConfig } from '@/types'
import TravelExpenseImportResultContent from '../components/TravelExpenseImportResultContent.tsx'
...
export function useTravelExpenseImportResultDialog() {
  const { displayCustom, displayClose, updateConfig } = useDialog()
  const { invalidateByPrefix } = useInvalidateQueries()
  const [isOpen, setIsOpen] = useState(false)
  const [result, setResult] = useState<TravelExpenseImportResult | null>(null)
  ...
  const loadResultFiles = useCallback(async (rawFiles: ResultFiles | string | null | undefined) => {
    ...
    const parsedResult: TravelExpenseImportResult = {
  ...
  const handleClose = useCallback(() => {
    ...
    invalidateByPrefix('payroll/travel-expenses')
  }, [invalidateByPrefix])
  ...
  const createConfig = useCallback((): DialogConfig => {
    return {
      ...
      content: result ? (
        <TravelExpenseImportResultContent
          result={result}
          activeTab={activeTab}
