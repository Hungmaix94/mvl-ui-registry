...
import {
  useTravelExpenseImportTemplate,
  useStartTravelExpenseImport,
  ...
  return (
    <Flex direction="column" gap="5" className="w-full">
      <MonthPicker
        ...
        label="Công tác phí tháng"
        value={month}
        onChange={onMonthChange}
        placeholder="Chọn tháng công tác phí"
        showYear
        required
      />
      <FileUpload
        ...
        purpose="payroll_travel_expense_import"
      />
    </Flex>
  )
}

export function useTravelExpenseImportUploadDialog() {
  ...
  const startImportMutation = useStartTravelExpenseImport()
  const {
    refetch: refetchTemplate,
    isFetching: isTemplateFetching,
    data: templateResponse,
  } = useTravelExpenseImportTemplate({ enabled: false })
  ...
  const handleConfirm = useCallback(async () => {
    ...
    if (!month) {
      const message = 'Vui lòng chọn công tác phí tháng'
      setUploadError(message)
      setError(message)
      throw { isValidationError: true }
    }
  ...
  const createConfig = useCallback(() => {
    return {
      title: 'Nhập danh sách Công tác phí',
      content: (
        <UploadContent
      ...
      leftFooterContent: ability.can('import_template', 'payroll.travel_expense') ? (
