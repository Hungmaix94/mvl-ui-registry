...
type SendEmailProgressDialogProps = {
  progress: number
...
export default function SendEmailProgressDialog(props: SendEmailProgressDialogProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-9 px-6 py-20">
      ...
      <div className="flex w-full flex-col items-end gap-5 text-center">
        {status === 'failure' ? (
          <>
            <p className="typo-h4 text-content-dark-1 w-full">Gửi mail lương thất bại</p>
            <p className="typo-body-lg-regular text-content-dark-3 w-full">
              {error || 'Đã xảy ra lỗi khi gửi mail lương'}
            </p>
          </>
        ) : status === 'success' ? (
          <>
            <p className="typo-h4 text-content-dark-1 w-full">Gửi mail lương thành công</p>
            <p className="typo-body-lg-regular text-content-dark-3 w-full">
              Email đã được gửi đến danh sách nhân viên
            </p>
          </>
        ) : (
          <>
            <p className="typo-h4 text-content-dark-1 w-full">Đang gửi mail lương...</p>
            <p className="typo-body-lg-regular text-content-dark-3 w-full">
      ...
      <div className="flex w-full items-center justify-center gap-4 px-6">
        {status === 'success' ? (
          <Button variant="primary" size="medium" onClick={onClose} className="w-[150px]">
            Đóng
          </Button>
        ) : status === 'failure' ? (
          <>
