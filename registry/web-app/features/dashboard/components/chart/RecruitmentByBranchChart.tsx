...
import { useBranchBreakdown } from '@/services'
...
type RecruitmentByBranchChartProps = { compact?: boolean }

function RecruitmentByBranchChart({ compact }: RecruitmentByBranchChartProps = {}) {
  ...
  const { data: apiData, isLoading } = useBranchBreakdown(apiParams)
  ...
  const handleDownloadChart = async () => {
    ...
    await exportElementToPdf(chartRef.current, {
      fileName: 'bieu-do-ty-le-tuyen-moi-theo-chi-nhanh.pdf',
      overlayMessage: 'Đang tạo biểu đồ Tỷ lệ tuyển mới theo chi nhánh...',
    })
  }
  ...
  return (
    <div
      ...
      data-api={ApiPaths.hrm_dashboard_charts_branch_breakdown_retrieve}
    >
      <DashboardChartTitle
        reportLink={APP_PATH.REPORT_RECRUITMENT_HIRED_CANDIDATE}
        title={'Tỷ lệ tuyển mới theo chi nhánh'}
...
export default RecruitmentByBranchChart
