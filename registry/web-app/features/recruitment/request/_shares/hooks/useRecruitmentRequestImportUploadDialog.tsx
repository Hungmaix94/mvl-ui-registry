...
import {
  useRecruitmentRequestImportTemplate,
  useStartRecruitmentRequestImport,
...
function UploadContent({ onFileChange, error }: UploadContentProps) {
  return (
    <FileUpload
      ...
      purpose="hrm_recruitment_request_import"
    />
  )
}

export function useRecruitmentRequestImportUploadDialog() {
  ...
  const confirmFilesMutation = useConfirmFiles({ skipInvalidateOnSuccess: true })
  const startImportMutation = useStartRecruitmentRequestImport()
  const {
    refetch: refetchTemplate,
    isFetching: isTemplateFetching,
    data: templateResponse,
  } = useRecruitmentRequestImportTemplate(undefined, { enabled: false })
  ...
  const createConfig = useCallback(() => {
    return {
      title: 'Nhập đề nghị tuyển dụng',
      content: <UploadContent onFileChange={handleFileChange} error={uploadError} />,
      leftFooterContent: ability.can('import_template', 'recruitment_request') ? (
