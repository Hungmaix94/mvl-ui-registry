...
import {
  useRecruitmentCandidateImportTemplate,
  useStartRecruitmentCandidateImport,
} from '@/services'
...
function UploadContent({ onFileChange, error }: UploadContentProps) {
  return (
    <FileUpload
      ...
      purpose="hrm_recruitment_candidate_import"
    />
  )
}

export function useRecruitmentCandidateImportUploadDialog() {
  ...
  const startImportMutation = useStartRecruitmentCandidateImport()
  const {
    refetch: refetchTemplate,
    isFetching: isTemplateFetching,
    data: templateResponse,
  } = useRecruitmentCandidateImportTemplate(undefined, { enabled: false })
  ...
  const createConfig = useCallback(() => {
    return {
      title: 'Nhập danh sách ứng viên tuyển dụng',
