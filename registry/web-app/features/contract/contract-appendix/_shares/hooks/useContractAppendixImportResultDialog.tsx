...
import { parseCsvContent } from '@/utils/import.ts'
import toastService from '@/services/toast-service.tsx'
import { extractErrorMessage } from '@/utils/error-utils'

import type { components } from '@/api/schema.ts'
import type { ContractAppendixImportResult } from '@/types/contract-appendix-import.ts'
import type { ImportJob } from '@/services/export-service.ts'
import type { DialogConfig } from '@/types'
import ContractAppendixImportResultContent from '@/features/contract/contract-appendix/_shares/components/ContractAppendixImportResultContent.tsx'
...
export function useContractAppendixImportResultDialog() {
  const { displayCustom, displayClose, updateConfig } = useDialog()
  const { invalidateByPrefix } = useInvalidateQueries()
  const [isOpen, setIsOpen] = useState(false)
  const [result, setResult] = useState<ContractAppendixImportResult | null>(null)
  ...
  const loadResultFiles = useCallback(async (rawFiles: ResultFiles | string | null | undefined) => {
    ...
    const parsedResult: ContractAppendixImportResult = {
      successRaw: successRaw ?? '',
      failureRaw: failureRaw ?? '',
      success: successRaw ? parseCsvContent(successRaw) : { headers: [], rows: [] },
      failure: failureRaw ? parseCsvContent(failureRaw) : { headers: [], rows: [] },
    }
  ...
  const handleClose = useCallback(() => {
    ...
    invalidateByPrefix('hrm/contract-appendices')
  }, [invalidateByPrefix])
  ...
  const createConfig = useCallback((): DialogConfig => {
    return {
      ...
      content: result ? (
        <ContractAppendixImportResultContent
          result={result}
          activeTab={activeTab}
  ...
  }, [activeTab, displayClose, handleClose, isLoading, result])
  ...
  const openResultDialog = useCallback((job: ImportJob) => {
