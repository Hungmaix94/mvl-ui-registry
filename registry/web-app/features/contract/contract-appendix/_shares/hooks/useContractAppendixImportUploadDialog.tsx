...
function UploadContent({ onFileChange, error }: UploadContentProps) {
  return (
    <FileUpload
      ...
      purpose="hrm_contract_appendix_import"
    />
  )
}

export function useContractAppendixImportUploadDialog() {
  ...
  const startImportMutation = useStartContractAppendixImport()
  const {
    refetch: refetchTemplate,
    isFetching: isTemplateFetching,
    data: templateResponse,
  } = useContractAppendixImportTemplate({ enabled: false })
  ...
  const createConfig = useCallback(() => {
    return {
      title: 'Nhập danh sách Phụ lục hợp đồng',
      content: <UploadContent onFileChange={handleFileChange} error={uploadError} />,
      leftFooterContent: ability.can('import_template', 'contract_appendix') ? (
