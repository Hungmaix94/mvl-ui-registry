...
type BulkJobTransferTableProps = {
  data: ProposalBulkJobTransfer[]
  isLoading: boolean
  error: Error | null
  pageCount: number
  pageSize: number
...
const BulkJobTransferTable = ({
  ...
  const columns: ColumnDef<ProposalBulkJobTransfer>[] = useMemo(
    () => [
      {
        accessorKey: 'code',
        header: 'Mã đề xuất',
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          const employee = getValue() as ProposalBulkJobTransfer['created_by']
          return (
            <Flex
              direction="column"
      ...
      {
        accessorKey: 'job_transfer_lines',
        header: 'Số nhân sự',
        cell: ({ getValue }) => {
          const lines = getValue() as ProposalBulkJobTransfer['job_transfer_lines']
          return <span className="text-content-dark-1 text-sm">{lines ? lines.length : 0}</span>
        },
        meta: { width: 'w-[110px]', align: 'center', sortable: false },
      },
      {
        accessorKey: 'job_transfer_effective_date',
      ...
      },
      {
        ...
        meta: { width: 'w-[130px]', align: 'center', sortable: true },
      },
    ],
  ...
  const actions = useProposalRowActions<ProposalBulkJobTransfer>({
    proposalType: ProposalType.bulk_job_transfer,
    onApproveSuccess: (record) => showBulkJobTransferApprovedInfo(displayCustom, record),
  })
...
export default BulkJobTransferTable
