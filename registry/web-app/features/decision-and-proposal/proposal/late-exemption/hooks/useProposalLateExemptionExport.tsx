...
import {
  ...
  type GetProposalsLateExemptionExportParams,
...
type ExportParams = NonNullable<GetProposalsLateExemptionExportParams>
...
type DurationTypeArray = NonNullable<ExportParams['late_exemption_duration_type__in']>

type ProposalFilterParams = Partial<GetProposalsLateExemptionParams> & {
  date_range?: { from?: Date; to?: Date }
  late_exemption_duration_type?: DurationTypeArray[number]
...
export function useProposalLateExemptionExport() {
  const { openExportDialog: baseOpenExportDialog, isExporting } = useExport<
    NonNullable<GetProposalsLateExemptionExportParams>
  >({
    exportFunction: (params) => getProposalMiscService().exportProposalsLateExemption(params),
    defaultFilename: 'proposals-late-exemption.xlsx',
  })

  const openExportDialog = useCallback(
    async (searchQuery: string, filterParams: ProposalFilterParams) => {
      const exportParams: GetProposalsLateExemptionExportParams = {
        async: true,
      ...
      if (filterParams?.late_exemption_duration_type) {
        exportParams.late_exemption_duration_type = filterParams.late_exemption_duration_type
      }
