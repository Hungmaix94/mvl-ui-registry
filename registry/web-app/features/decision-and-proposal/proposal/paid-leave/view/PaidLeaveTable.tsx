...
type PaidLeaveTableProps = {
  data: ProposalPaidLeave[]
  isLoading: boolean
  error: Error | null
  pageCount: number
  pageSize: number
...
const PaidLeaveTable = ({
  ...
  // Columns
  const columns: ColumnDef<ProposalPaidLeave>[] = useMemo(
    () => [
      {
        accessorKey: 'code',
        header: 'Mã đề xuất',
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const startDate = row.original.paid_leave_start_date
          const endDate = row.original.paid_leave_end_date
          const display =
            startDate && endDate
      ...
      {
        accessorKey: 'paid_leave_shift',
        header: 'Buổi',
        cell: ({ getValue }) => {
          const value = getValue() as ProposalPaidLeave['paid_leave_shift']
  ...
  const actions = useProposalRowActions<ProposalPaidLeave>({
    proposalType: ProposalType.paid_leave,
  })
