...
import {
  getProposalLeaveService,
  type GetProposalsMaternityLeaveExportParams,
...
type ExportParams = NonNullable<GetProposalsMaternityLeaveExportParams>
...
type ProposalFilterParams = Partial<GetProposalsMaternityLeaveParams> & {
...
export function useProposalMaternityLeaveExport() {
  const { openExportDialog: baseOpenExportDialog, isExporting } = useExport<
    NonNullable<GetProposalsMaternityLeaveExportParams>
  >({
    exportFunction: (params) => getProposalLeaveService().exportProposalsMaternityLeave(params),
    defaultFilename: 'proposals-maternity-leave.xlsx',
  })

  const openExportDialog = useCallback(
    async (searchQuery: string, filterParams: ProposalFilterParams) => {
      const exportParams: GetProposalsMaternityLeaveExportParams = {
        async: true,
