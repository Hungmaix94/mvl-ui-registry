...
type ReturnToWorkTableProps = {
  data: ProposalReturnToWork[]
  isLoading: boolean
  error: Error | null
  pageCount: number
  pageSize: number
...
const ReturnToWorkTable = ({
  ...
  onClearFilter,
  hasFilter,
}: ReturnToWorkTableProps) => {
  const statusMapping = useProposalStatusMapping()

  const columns: ColumnDef<ProposalReturnToWork>[] = useMemo(
    () => [
      {
        accessorKey: 'code',
        header: 'Mã đề xuất',
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          const employee = getValue() as ProposalReturnToWork['created_by']
          return (
            <Flex
      ...
      {
        accessorKey: 'return_to_work_date',
        header: 'Ngày quay lại làm việc',
        cell: ({ getValue }) => {
          const date = getValue() as string | null | undefined
          const display = date ? formatDate(date) : '-'
          return (
  ...
  const actions = useProposalRowActions<ProposalReturnToWork>({
    proposalType: ProposalType.return_to_work,
  })
...
export default ReturnToWorkTable
