...
type PostMaternityBenefitTableProps = {
  data: ProposalPostMaternityBenefits[]
  isLoading: boolean
  error: Error | null
  pageCount: number
  pageSize: number
...
const PostMaternityBenefitTable = ({
  data,
  isLoading,
  error,
  pageCount,
  ...
  onClearFilter,
  hasFilter,
}: PostMaternityBenefitTableProps) => {
  const statusMapping = useProposalStatusMapping()

  // Columns
  const columns: ColumnDef<ProposalPostMaternityBenefits>[] = useMemo(
    () => [
      {
        accessorKey: 'code',
        header: 'Mã đề xuất',
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          const employee = getValue() as ProposalPostMaternityBenefits['created_by']
          return (
            <Flex
              direction="column"
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const startDate = row.original.post_maternity_benefits_start_date
          const endDate = row.original.post_maternity_benefits_end_date
          const display =
            startDate && endDate
      ...
      },
      {
        ...
        meta: { width: 'w-[150px]', sortable: true, align: 'center' },
      },
    ],
    [statusMapping]
  )

  const actions = useProposalRowActions<ProposalPostMaternityBenefits>({
    proposalType: ProposalType.post_maternity_benefits,
  })
...
export default PostMaternityBenefitTable
