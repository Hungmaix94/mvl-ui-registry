...
import {
  ...
  type GetProposalsUnpaidLeaveExportParams,
...
type ExportParams = NonNullable<GetProposalsUnpaidLeaveExportParams>
...
export function useProposalUnpaidLeaveExport() {
  const { openExportDialog: baseOpenExportDialog, isExporting } = useExport<
    NonNullable<GetProposalsUnpaidLeaveExportParams>
  >({
    exportFunction: (params) => getProposalLeaveService().exportProposalsUnpaidLeave(params),
    defaultFilename: 'proposals-unpaid-leave.xlsx',
  })

  const openExportDialog = useCallback(
    async (searchQuery: string, filterParams: ProposalFilterParams) => {
      const exportParams: GetProposalsUnpaidLeaveExportParams = {
        async: true,
