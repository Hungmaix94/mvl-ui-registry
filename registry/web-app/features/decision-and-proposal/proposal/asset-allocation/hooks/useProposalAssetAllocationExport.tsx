...
import {
  ...
  type GetProposalsAssetAllocationExportParams,
...
type ExportParams = NonNullable<GetProposalsAssetAllocationExportParams>
...
type ProposalFilterParams = Partial<GetProposalsAssetAllocationParams> & {
...
export function useProposalAssetAllocationExport() {
  const { openExportDialog: baseOpenExportDialog, isExporting } = useExport<
    NonNullable<GetProposalsAssetAllocationExportParams>
  >({
    exportFunction: (params) => getProposalMiscService().exportProposalsAssetAllocation(params),
    defaultFilename: 'proposals-asset-allocation.xlsx',
  })

  const openExportDialog = useCallback(
    async (searchQuery: string, filterParams: ProposalFilterParams) => {
      const exportParams: GetProposalsAssetAllocationExportParams = {
        async: true,
