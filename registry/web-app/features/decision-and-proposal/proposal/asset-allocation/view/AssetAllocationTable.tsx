...
const AssetAllocationTable = ({
  ...
  // Columns
  const columns: ColumnDef<ProposalAssetAllocation>[] = useMemo(
    () => [
      {
        accessorKey: 'code',
        header: 'Mã đề xuất',
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          const employee = getValue() as ProposalAssetAllocation['created_by']
          return (
            <Flex
      ...
      },
      {
        ...
        header: 'Trạng thái',
        cell: ({ getValue }) => {
          const colored = getValue() as ProposalAssetAllocation['colored_proposal_status']
          if (!colored.value) {
  ...
  const actions = useProposalRowActions<ProposalAssetAllocation>({
