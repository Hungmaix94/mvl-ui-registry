...
import {
  ...
  type GetProposalsDeviceChangeExportParams,
...
type ExportParams = NonNullable<GetProposalsDeviceChangeExportParams>
...
type ProposalFilterParams = Partial<GetProposalsDeviceChangeParams> & {
...
export function useProposalDeviceChangeExport() {
  const { openExportDialog: baseOpenExportDialog, isExporting } = useExport<
    NonNullable<GetProposalsDeviceChangeExportParams>
  >({
    exportFunction: (params) => getProposalMiscService().exportProposalsDeviceChange(params),
    defaultFilename: 'proposals-device-change.xlsx',
  })

  const openExportDialog = useCallback(
    async (searchQuery: string, filterParams: ProposalFilterParams) => {
      const exportParams: GetProposalsDeviceChangeExportParams = {
        async: true,
