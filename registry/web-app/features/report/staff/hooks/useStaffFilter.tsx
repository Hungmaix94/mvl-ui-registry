...
import type { GetEmployeeResignedBreakdownReportParams } from '@/services'
import { startOfMonth } from 'date-fns'
import useAppConstant from '@/hooks/useAppConstant.ts'
import { APP_CONSTANT_KEY } from '@/constants/app-constant-key.ts'
import { BlockType, ResignedBreakdownPeriodType } from '@/constants/api-schema-aliases'

const getDefaultDateParams = (periodType: ResignedBreakdownPeriodType) => {
  if (periodType === ResignedBreakdownPeriodType.month) {
...
type UseStaffTurnoverFilterResult = {
  filterParams: GetEmployeeResignedBreakdownReportParams
...
export const useStaffFilter = (
  periodType: ResignedBreakdownPeriodType
): UseStaffTurnoverFilterResult => {
  ...
  const [filterParams, setFilterParams] = useState<GetEmployeeResignedBreakdownReportParams>(
    () => ({
      period_type: periodType,
      ...getDefaultDateParams(periodType),
    })
  )
  ...
  useEffect(() => {
    setFilterParams((prev) => {
      const defaultDates = getDefaultDateParams(periodType)
      return {
        ...prev,
        period_type: periodType,
        ...(prev.from_date || prev.to_date ? {} : defaultDates),
      }
    })
  }, [periodType])
  ...
  const handleApplyFilters = useCallback(
    ...
    [displayClose, invalidateQueries, periodType]
  )
