...
export interface KPIPeriodSummaryData {
  readonly id: number
...
type KPIPeriodSummaryTableProps = {
  data: KPIPeriodSummaryData[]
  isLoading: boolean
  error: Error | null
  pageCount: number
  pageSize: number
...
const KPIPeriodSummaryTable = ({
  ...
  onSortingChange,
  hasFilter = false,
}: KPIPeriodSummaryTableProps) => {
  const navigate = useNavigate()

  const columns = useMemo<ColumnDef<KPIPeriodSummaryData>[]>(
    () => [
      {
        accessorKey: 'month',
        header: 'Kỳ đánh giá',
        ...
        cell: ({ row }) => (
          <Button
            ...
            onClick={() =>
              navigate(
                {
                  pathname: APP_PATH.KPI_PERIOD_SUMMARY_DETAIL.replace(
                    ':id',
...
export default KPIPeriodSummaryTable
