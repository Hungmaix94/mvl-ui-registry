...
import { useCustomerImportTemplate, useStartCustomerImport } from '@/services'
...
function UploadContent({ onFileChange, error }: UploadContentProps) {
  return (
    <FileUpload
      ...
      purpose="sales_customer_import"
    />
  )
}

export function useCustomerImportUploadDialog() {
  ...
  const startImportMutation = useStartCustomerImport()
  const {
    refetch: refetchTemplate,
    isFetching: isTemplateFetching,
    data: templateResponse,
  } = useCustomerImportTemplate({ enabled: false })
  ...
  const createConfig = useCallback(() => {
    return {
      ...
      leftFooterContent: ability.can('create', 'customer') ? (
