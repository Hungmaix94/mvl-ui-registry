...
import {
  useEmployeeCertificatesImportTemplate,
  useStartEmployeeCertificatesImport,
...
function UploadContent({ onFileChange, error }: UploadContentProps) {
  return (
    <FileUpload
      ...
      purpose="hrm_employee_certificate_import"
    />
  )
}

export function useEmployeeCertificateImportUploadDialog() {
  ...
  const startImportMutation = useStartEmployeeCertificatesImport()
  const {
    refetch: refetchTemplate,
    isFetching: isTemplateFetching,
    data: templateResponse,
  } = useEmployeeCertificatesImportTemplate({ enabled: false })
  ...
  const createConfig = useCallback(() => {
    return {
      title: 'Nhập danh sách Bằng cấp, chứng chỉ môi giới',
      content: <UploadContent onFileChange={handleFileChange} error={uploadError} />,
      leftFooterContent: ability.can('import_template', 'employee_certificate') ? (
