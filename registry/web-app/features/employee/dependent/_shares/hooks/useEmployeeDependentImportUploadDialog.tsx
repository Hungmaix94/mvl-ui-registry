...
import {
  useEmployeeDependentsImportTemplate,
  useStartEmployeeDependentsImport,
...
function UploadContent({ onFileChange, error }: UploadContentProps) {
  return (
    <FileUpload
      ...
      purpose="hrm_employee_dependent_import"
    />
  )
}

export function useEmployeeDependentImportUploadDialog() {
  ...
  const startImportMutation = useStartEmployeeDependentsImport()
  const {
    refetch: refetchTemplate,
    isFetching: isTemplateFetching,
    data: templateResponse,
  } = useEmployeeDependentsImportTemplate({ enabled: false })
  ...
  const createConfig = useCallback(() => {
    return {
      title: 'Nhập danh sách Người phụ thuộc',
      content: <UploadContent onFileChange={handleFileChange} error={uploadError} />,
      leftFooterContent: ability.can('import_template', 'employee_dependent') ? (
