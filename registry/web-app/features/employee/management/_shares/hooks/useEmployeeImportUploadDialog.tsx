...
import { useEmployeeImportTemplate, useStartEmployeeImport } from '@/services'
...
function UploadContent({ onFileChange, error }: UploadContentProps) {
  return (
    <FileUpload
      ...
      purpose="hrm_employee_import"
    />
  )
}

export function useEmployeeImportUploadDialog() {
  ...
  const startImportMutation = useStartEmployeeImport()
  const {
    refetch: refetchTemplate,
    isFetching: isTemplateFetching,
    data: templateResponse,
  } = useEmployeeImportTemplate(undefined, { enabled: false })
  ...
  const createConfig = useCallback(() => {
    return {
      title: 'Nhập danh sách Hồ sơ nhân viên',
      content: <UploadContent onFileChange={handleFileChange} error={uploadError} />,
      leftFooterContent: ability.can('import_template', 'employee') ? (
