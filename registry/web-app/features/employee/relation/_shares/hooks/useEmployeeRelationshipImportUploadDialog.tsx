...
function UploadContent({ onFileChange, error }: UploadContentProps) {
  return (
    <FileUpload
      ...
      purpose="hrm_employee_relationship_import"
    />
  )
}

export function useEmployeeRelationshipImportUploadDialog() {
  const ability = useAbility()

  const { displayCustom, displayClose, setLoading, setError } = useDialog()
  const [isOpen, setIsOpen] = useState(false)
  const [fileToken, setFileToken] = useState<string | null>(null)
  const [uploadError, setUploadError] = useState<string | null>(null)

  const confirmFilesMutation = useConfirmFiles()
  const startImportMutation = useStartEmployeeRelationshipsImport()
  const {
  ...
  }, [fileToken, confirmFilesMutation, startImportMutation, setLoading, setError, displayClose])

  const createConfig = useCallback(() => {
    return {
      title: 'Nhập dữ liệu quan hệ nhân thân',
      content: <UploadContent onFileChange={handleFileChange} error={uploadError} />,
      leftFooterContent: ability.can('import_template', 'employee_relationship') ? (
  ...
  }, [isOpen, displayCustom, createConfig])
