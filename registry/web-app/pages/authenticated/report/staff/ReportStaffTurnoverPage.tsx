...
): GetEmployeeResignedBreakdownReportParams {
  // Period type - always required
  const periodType = searchParams.get('period_type')
  const params: GetEmployeeResignedBreakdownReportParams = {
...
function ReportStaffTurnoverPage() {
  ...
  // API call
  const { data, isLoading } = useEmployeeResignedBreakdownReport(apiParams, {
  ...
  const handleExportTable = useCallback(() => {
    ...
    const fileName =
      filterTitle && filterTitle !== 'Toàn công ty'
        ? `Báo cáo Số lượng Nhân sự nghỉ việc - ${filterTitle}`
        : 'Báo cáo Số lượng Nhân sự nghỉ việc'
    ...
    <>
      ...
      <Flex direction={'column'} align={'start'} px={'7'}>
        ...
        <Flex direction={'column'} py={'4'} className={'w-full'} gap={'4'}>
          <StaffChartTitle
            title={`Báo cáo Số lượng Nghỉ việc${filterTitle ? ' - ' + filterTitle : ''}`}
...
export default ReportStaffTurnoverPage
