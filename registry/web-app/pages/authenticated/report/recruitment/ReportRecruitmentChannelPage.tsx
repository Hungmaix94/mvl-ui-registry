...
import ReportRecruitmentChannelTable from '@/features/report/recruitment/channel/view/ReportRecruitmentChannelTable'
...
import {
  ...
  useExportRecruitmentChannelReport,
...
): GetRecruitmentChannelReportParams {
  const params: GetRecruitmentChannelReportParams = {}
...
const ReportRecruitmentChannelPage = () => {
  ...
  // API call
  const { data, isLoading } = useRecruitmentChannelReport(apiParams, {
  ...
  const activeFilterCount = useMemo(() => {
    let count = 0
    if (searchParams.get('from_date') || searchParams.get('to_date')) count++
    if (validatedBranchId) count++
    if (validatedBlockId) count++
    if (validatedDepartmentId) count++
    return count
  }, [searchParams, validatedBranchId, validatedBlockId, validatedDepartmentId])
  ...
  // Export is generated server-side (styled XLSX) via the recruitment-channel export endpoint.
  const { openExportDialog } = useExportRecruitmentChannelReport()
...
export default ReportRecruitmentChannelPage
