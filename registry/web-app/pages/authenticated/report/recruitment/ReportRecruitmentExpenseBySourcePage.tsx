...
import ReportRecruitmentExpenseTable from '@/features/report/recruitment/expense/view/ReportRecruitmentExpenseTable'
...
import {
  type GetCostBySourceReportParams,
  useCostBySourceReport,
  useExportCostBySourceReport,
...
): GetCostBySourceReportParams {
  const params: GetCostBySourceReportParams = {}
...
const ReportRecruitmentExpenseBySourcePage = () => {
  ...
  }, [searchParams, isUrlReady, isFilterValidationLoading, hasValidDateRange, validatedBranchId])

  const reportQueryEnabled =
    isUrlReady && !isFilterValidationLoading && hasValidDateRange && !!apiParams

  // API call
  const { data, isLoading } = useCostBySourceReport(apiParams, {
  ...
  const handleApplyFilter = useCallback(() => {
    ...
    // PAID is the server default — keep it out of the URL so the existing
    // "no param → đã chi" behaviour is preserved and the filter badge only
    // reflects an explicit non-default choice.
    if (formData.payment_status && formData.payment_status !== ReportPaymentStatus.PAID) {
      newParams.set('payment_status', formData.payment_status)
    }
    setSearchParams(newParams, { replace: true })
  ...
  const activeFilterCount = useMemo(() => {
    ...
    return count
  }, [urlParams.fromDate, urlParams.toDate, validatedBranchId, urlParams.paymentStatus])
  ...
  const filterOrg = useMemo(() => {
    ...
    return title
  }, [branchQuery.data?.name, isBranchValid])

  // Export is generated server-side (styled XLSX) via the cost-by-source export endpoint.
  const { openExportDialog } = useExportCostBySourceReport()
  ...
  const formInitialValues = useMemo(
    () => ({
      ...
      paymentStatus: urlParams.paymentStatus ?? ReportPaymentStatus.PAID,
    }),
    [urlParams.fromDate, urlParams.toDate, validatedBranchId, urlParams.paymentStatus]
  )
    ...
    <>
      ...
      <Flex flexGrow="1" direction="column" gap="0" className="pb-6">
        <Box className="px-10 pb-4">
          <Flex direction={'column'} gap="2" width="full" wrap="wrap" align="start">
            <Flex gap={'2'}>
              {filterDateRange && (
                <>
              ...
              {paymentStatusLabel && (
                <>
                  <Text className="typo-body-lg-medium text-content-dark-2">
...
export default ReportRecruitmentExpenseBySourcePage
