...
import { RecruitmentChannelTable } from '@/features/recruitment/channel'
...
import {
  type GetRecruitmentChannelsParams,
  useRecruitmentChannels,
...
): NonNullable<GetRecruitmentChannelsParams> {
  const params: NonNullable<GetRecruitmentChannelsParams> = {}
...
const RecruitmentChannelPage = () => {
  ...
  const { data, isLoading, error, isFetching, isRefetching } = useRecruitmentChannels(
    apiParams ?? {},
  ...
  const handleCreateNew = useCallback(() => {
    navigate(APP_PATH.RECRUITMENT_CHANNEL_CREATE, {
      state: { from: window.location.pathname + window.location.search },
    })
  }, [navigate])

  const handleDeleteRecruitmentChannel = useCallback(
  ...
  return (
    <>
      <PageTitle
        ...
        searchPlaceholder="Tìm kiếm theo mã kênh, tên kênh"
        searchClassName="!w-[350px]"
        handleCreateNew={ability.can('create', 'recruitment_channel') ? handleCreateNew : undefined}
      />
      <Flex flexGrow={'1'} direction="column" gap="4" className={'pb-6'}>
        <RecruitmentChannelTable
          data={tableData}
          ...
          onDeleteRecruitmentChannel={handleDeleteRecruitmentChannel}
...
export default RecruitmentChannelPage
