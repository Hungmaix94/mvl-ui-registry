...
export default function ProposalStatutoryLeaveDetailPage() {
  ...
  const handleShowHistory = useCallback(() => {
    if (proposalId && proposalId > 0) {
      const path = APP_PATH.PROPOSAL_STATUTORY_LEAVE_HISTORY.replace(':id', proposalId.toString())
      navigate(path)
    }
  }, [navigate, proposalId])
  ...
  return (
    <>
      ...
      <DetailPageWrapper
        ...
        isError={isError}
        hasPermission={ability.can('retrieve', 'proposal_statutory_leave')}
      >
        {proposal && (
