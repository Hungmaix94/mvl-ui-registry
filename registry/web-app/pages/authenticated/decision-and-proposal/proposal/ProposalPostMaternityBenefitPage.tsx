...
import PostMaternityBenefitTable from '@/features/decision-and-proposal/proposal/post-maternity-benefit/view/PostMaternityBenefitTable.tsx'
import {
  useProposalsPostMaternityBenefits,
  type GetProposalsPostMaternityBenefitsParams,
...
import { useProposalPostMaternityBenefitExport } from '@/features/decision-and-proposal/proposal/post-maternity-benefit/hooks/useProposalPostMaternityBenefitExport.tsx'
...
function buildApiParamsFromUrl(
...
): NonNullable<GetProposalsPostMaternityBenefitsParams> {
  const params: NonNullable<GetProposalsPostMaternityBenefitsParams> = {}
  ...
  // Status - multi params (always use __in array filter)
  const statuses = searchParams.getAll('status')
  if (statuses.length > 0) {
    params.proposal_status__in = statuses
  }

  // Verifier Status - multi params (always use __in array filter)
  const verifierStatuses = searchParams.getAll('verifier_status')
  if (verifierStatuses.length > 0) {
...
export default function ProposalPostMaternityBenefitPage() {
  ...
  const { openExportDialog } = useProposalPostMaternityBenefitExport()
  ...
  const apiParams = useMemo(() => {
    ...
    // Filter status values - only include valid ones, always using __in array filter
  ...
  const {
    ...
    isFetching,
    isRefetching,
  } = useProposalsPostMaternityBenefits(apiParams, {
  ...
  return (
    <>
      <PageTitle
        ...
        handleExportBtnFull={
          ability.can('export', 'proposal_post_maternity_benefits') ? handleExport : undefined
        }
      />
      <Flex flexGrow={'1'} direction="column" gap="4" className={'pb-6'}>
        <PostMaternityBenefitTable
          data={tableData}
