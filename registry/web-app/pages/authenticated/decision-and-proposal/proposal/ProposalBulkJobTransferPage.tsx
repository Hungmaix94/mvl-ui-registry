...
import BulkJobTransferTable from '@/features/decision-and-proposal/proposal/bulk-job-transfer/view/BulkJobTransferTable.tsx'
import {
  useProposalsBulkJobTransfer,
  type GetProposalsBulkJobTransferParams,
...
import { useProposalBulkJobTransferExport } from '@/features/decision-and-proposal/proposal/bulk-job-transfer/hooks/useProposalBulkJobTransferExport.tsx'
...
import { JobTransferStatus } from '@/constants/api-schema-aliases'
const TRANSFER_STATUS_VALUES = Object.values(JobTransferStatus)

/** Narrow a raw URL string to the typed transfer_status enum, or undefined if invalid. */
function toTransferStatus(value: string | null): JobTransferStatus | undefined {
  return TRANSFER_STATUS_VALUES.find((status) => status === value)
}
...
function parseFilterParamsFromUrl(searchParams: URLSearchParams): FilterParams {
  ...
  // Parse transfer_status (single value)
  const transferStatus = toTransferStatus(searchParams.get('transfer_status'))
  if (transferStatus) {
...
function buildApiParamsFromUrl(
...
): NonNullable<GetProposalsBulkJobTransferParams> {
  const params: NonNullable<GetProposalsBulkJobTransferParams> = {}
  ...
  // Transfer Status - single value (validated against enum)
  const transferStatus = toTransferStatus(searchParams.get('transfer_status'))
  if (transferStatus) {
...
export default function ProposalBulkJobTransferPage() {
  ...
  const { openExportDialog } = useProposalBulkJobTransferExport()
  ...
  const {
    ...
    isFetching,
    isRefetching,
  } = useProposalsBulkJobTransfer(apiParams, {
  ...
  const handleApplyFilter = useCallback(() => {
    ...
    // Add transfer_status (single value)
    if (formData.transfer_status) {
      newParams.set('transfer_status', formData.transfer_status)
    }
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (currentFilterParams.transferStatus) count++
    return count
  }, [currentFilterParams])
  ...
  const formInitialValues = useMemo(() => {
    return {
      ...
      transfer_status: currentFilterParams.transferStatus || undefined,
    }
  }, [currentFilterParams])
  ...
  const handleCreateNew = useCallback(() => {
    navigate(APP_PATH.PROPOSAL_BULK_JOB_TRANSFER_CREATE)
  }, [navigate])
  ...
  return (
    <>
      <PageTitle
        ...
        handleExportBtnFull={
          ability.can('export', 'proposal_bulk_job_transfer') ? handleExport : undefined
        }
        handleCreateNew={
          ability.can('create', 'proposal_bulk_job_transfer') ? handleCreateNew : undefined
        }
      />
      <Flex flexGrow={'1'} direction="column" gap="4" className={'pb-6'}>
        <BulkJobTransferTable
          data={tableData}
