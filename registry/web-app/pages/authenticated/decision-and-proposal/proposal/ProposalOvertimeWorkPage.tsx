...
import OvertimeWorkTable from '@/features/decision-and-proposal/proposal/overtime-work/view/OvertimeWorkTable.tsx'
import {
...
import { useProposalOvertimeWorkExport } from '@/features/decision-and-proposal/proposal/overtime-work/hooks/useProposalOvertimeWorkExport.tsx'
...
type FilterParams = {
  ...
  overtimeDateRange?: { from?: Date; to?: Date } | null
  branchId?: number
  blockId?: number
  departmentId?: number
  positionId?: number
...
function parseFilterParamsFromUrl(searchParams: URLSearchParams): FilterParams {
  ...
  const otFromDate = searchParams.get('ot_from')
  const otToDate = searchParams.get('ot_to')
  if (otFromDate || otToDate) {
    try {
      params.overtimeDateRange = {
        from: otFromDate ? parse(otFromDate, DATE_SERVER_FORMAT, new Date()) : undefined,
        to: otToDate ? parse(otToDate, DATE_SERVER_FORMAT, new Date()) : undefined,
      }
    } catch {
      // If parsing fails, leave as undefined
    }
...
function buildApiParamsFromUrl(
...
): NonNullable<GetProposalsOvertimeWorkParams> {
  const params: NonNullable<GetProposalsOvertimeWorkParams> = {}
  ...
  // Overtime entry date range - proposals with at least one OT entry within range
  const otFromDate = searchParams.get('ot_from')
  if (otFromDate) {
    params.overtime_date_from = otFromDate
  }

  const otToDate = searchParams.get('ot_to')
  if (otToDate) {
    params.overtime_date_to = otToDate
  }
...
export default function ProposalOvertimeWorkPage() {
  ...
  const { openExportDialog } = useProposalOvertimeWorkExport()
  ...
  const {
    ...
    isFetching,
    isRefetching,
  } = useProposalsOvertimeWork(apiParams, {
  ...
  const handleApplyFilter = useCallback(() => {
    ...
    if (formData.overtime_date_range?.from) {
      newParams.set('ot_from', formatDateToApi(formData.overtime_date_range.from))
    }
    if (formData.overtime_date_range?.to) {
      newParams.set('ot_to', formatDateToApi(formData.overtime_date_range.to))
    }
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (currentFilterParams.overtimeDateRange?.from || currentFilterParams.overtimeDateRange?.to)
      count++
  ...
  const formInitialValues = useMemo(() => {
    return {
      ...
      overtime_date_range: currentFilterParams.overtimeDateRange || null,
  ...
  const handleExport = useCallback(() => {
    // Build export params from current URL params
    const exportParams: any = {
      ...
      proposal_date__lte: currentFilterParams.dateRange?.to
        ...
        : undefined,
      overtime_date_from: currentFilterParams.overtimeDateRange?.from
        ? formatDateToApi(currentFilterParams.overtimeDateRange.from)
        : undefined,
      overtime_date_to: currentFilterParams.overtimeDateRange?.to
        ? formatDateToApi(currentFilterParams.overtimeDateRange.to)
        : undefined,
    }
  ...
  return (
    <>
      <PageTitle
        ...
        handleExportBtnFull={
          ability.can('export', 'proposal_overtime_work') ? handleExport : undefined
        }
      />
