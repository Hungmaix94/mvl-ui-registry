...
export default function ProposalPostMaternityBenefitDetailPage() {
  ...
  const handleShowHistory = useCallback(() => {
    if (proposalId && proposalId > 0) {
      const path = APP_PATH.PROPOSAL_POST_MATERNITY_BENEFIT_HISTORY.replace(
        ':id',
        proposalId.toString()
      )
      navigate(path)
  ...
  return (
    <>
      ...
      <DetailPageWrapper
        ...
        isError={isError}
        hasPermission={ability.can('retrieve', 'proposal_post_maternity_benefits')}
      >
        {proposal && (
