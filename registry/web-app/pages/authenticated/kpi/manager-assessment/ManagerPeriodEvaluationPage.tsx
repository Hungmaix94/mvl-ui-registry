...
import { usePayrollKPIPeriodsManager } from '@/features/kpi/services/kpi-assessment-service'
...
const ManagerPeriodEvaluationPage = () => {
  ...
  const { data, isLoading } = usePayrollKPIPeriodsManager({
    ...
    () => [
      {
        ...
        onClick: (record: any) => {
          navigate(APP_PATH.KPI_MANAGER_PERIOD_EVALUATION_DETAIL.replace(':id', String(record.id)))
        },
        show: () => ability.can('retrieve', 'employee_manager_assessment'),
      },
    ],
    [ability, navigate]
  )
  ...
  return (
    <>
      <PageTitle
        title="Phiếu đánh giá KPI theo kỳ (Quản lý)"
        breadcrumb={[
          {
            label: 'Đánh giá KPI',
            href: APP_PATH.HOME,
          },
          {
            ...
            href: APP_PATH.KPI_MANAGER_PERIOD_EVALUATION,
          },
        ]}
        searchValue={search}
...
export default ManagerPeriodEvaluationPage
