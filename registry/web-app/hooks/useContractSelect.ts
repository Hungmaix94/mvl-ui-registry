...
type UseContractSelectOptions = {
  pageSize?: number
  additionalParams?: GetContractsParams | (() => GetContractsParams)
}

export function useContractSelect(options: UseContractSelectOptions = {}) {
  ...
  const loadContractOptions = useCallback(
    async (params: LoadOptionsParams): Promise<LoadOptionsResult<SelectOption>> => {
      ...
      try {
        ...
        // Build API params for contracts query
        const apiParams: GetContractsParams = {
          page: params.page,
        ...
        const queryKey = QUERY_KEYS.HRM.CONTRACTS.LIST(apiParams)
        const paginatedData = await queryClient.fetchQuery({
          queryKey,
          queryFn: () => getContractService().getContracts(apiParams),
          staleTime: 1000 * 60 * 5, // 5 minutes cache
        })

        // getPaginated returns TPaginatedData directly: { count, next, previous, results }
        if (!paginatedData || !paginatedData.results) {
          console.warn('No results in contracts API response:', paginatedData)
          return {
            items: [],
            hasNextPage: false,
            nextPage: null,
        ...
        // Map contracts to SelectOption format
        const items: SelectOption[] = paginatedData.results.map((contract: ContractListItem) => {
          return {
  ...
  const loadInitialContractOptions = useCallback(
    async (values: (string | number)[]): Promise<SelectOption[]> => {
      ...
      try {
        // Fetch by ID using getContract endpoint
        const fetchPromises = values.map(async (id) => {
          try {
            const contract = await queryClient.fetchQuery({
              queryKey: QUERY_KEYS.HRM.CONTRACTS.DETAIL(Number(id)),
              queryFn: () => getContractService().getContract(Number(id)),
              staleTime: 1000 * 60 * 5, // 5 minutes cache
            })
        ...
        console.error('Error loading initial contract options:', error)
